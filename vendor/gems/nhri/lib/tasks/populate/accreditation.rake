    ...
    namespace :internal_documents do
      ...
      task :populate => "nhri:accreditation:internal_documents:depopulate" do
        ...
        titles.each do |title|
          ...
          doc = FactoryBot.create(:accreditation_required_document, :revision => first_doc_rev.to_s, :title => title, :original_filename => rand_filename)
          dgid = doc.document_group_id
          5.times do |i|
            current_doc_rev -= 0.1
            current_doc_rev = current_doc_rev.round(1)
            FactoryBot.create(:accreditation_required_document, :document_group_id => dgid, :revision => current_doc_rev.to_s, :title => title, :original_filename => rand_filename)
          end
        end
      end
    end
