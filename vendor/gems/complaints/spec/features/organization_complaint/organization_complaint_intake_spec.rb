...
feature "organization complaint duplicate check", :js => true do
  include LoggedInEnAdminUserHelper # sets up logged in admin user
  ...
  it "should have fields disabled that are not relevant for duplicate check" do
    disabled_text_fields = %w[date_received title physical_address postal_address city province postal_code
                              contact_phone contact_cell_phone contact_fax complaint_details desired_outcome
                              assignee ]
  ...
  it "should show duplicates matching the entered organization name" do
  ...
  it "should show duplicates matching the entered organization registration number" do
    fill_in('organization_registration_number', with: "12341234")
    page.find(".btn#check_dupes").click
    wait_for_ajax
    expect(page).to have_selector('h4.modal-title', text: "Possible duplicates")
    expect(page).to have_selector('#agency_match_list', text: "no possible duplicates for agency")
