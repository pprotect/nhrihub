...
describe "" do
  ...
  it "should understand its parent's index" do
...
describe "delete a planned result" do
  before do
    @spr = StrategicPriority.create(:priority_level => '1', :description => "foo")
    @p1 = PlannedResult.create(:description => "foo", :strategic_priority => @spr)
    @o11 = Outcome.create(:planned_result_id => @p1.id, :description => "bar")
    @o12 = Outcome.create(:planned_result_id => @p1.id, :description => "bar")
    @p2 = PlannedResult.create(:description => "foo", :strategic_priority => @spr)
    @o21 = Outcome.create(:planned_result_id => @p2.id, :description => "bar")
    @o22 = Outcome.create(:planned_result_id => @p2.id, :description => "bar")
  end
  ...
  describe "delete a high priority outcome" do
    ...
    it "should adjust lower priority outcomes" do
      ...
      expect(@o21.reload.index).to eq [1,1,1]
      expect(@o22.reload.index).to eq [1,1,2]
      expect(@o23.reload.index).to eq [1,1,3]
